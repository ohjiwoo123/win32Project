...
enum SHAPE {X_shape,O_shape};

LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    static int color = RED;
    static int shape = O_shape;
    switch (message)
    {
    // 문자 관련 
    case WM_CHAR:
    ...
    case WM_PAINT:
    {
        ...
        HBRUSH hRed, hGreen, hBlue, NowBrush;
        ...
        switch (color)
        {
        case RED: NowBrush = hRed; break;
        case GREEN: NowBrush = hGreen; break;
        default: NowBrush = hBlue; break;
        }

        HBRUSH hOldBr = (HBRUSH)SelectObject(hdc, NowBrush);
        SelectObject(hdc, NowBrush);
        Rectangle(hdc, 0, 0, 100, 100);

        if (shape == X_shape) // X 그리기
        {
            MoveToEx(hdc, 10, 10, NULL); LineTo(hdc, 90, 90);
            MoveToEx(hdc, 90, 10, NULL); LineTo(hdc, 10, 90);
        }
        else if (shape == O_shape) // O 그리기 
        {
